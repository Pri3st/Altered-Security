...
- Moreover, with the NT hash of `DCORP-MGMT$` we can craft a TGS for `cifs/dcorp-mgmt.dollarcorp.moneycorp.local` to be able to dump hashes like in [[1. CRTP/2. Windows Tools/dollarcorp.moneycorp.local/3. Domain Domination/Path 8 - RBCD|Path 8 - RBCD]]