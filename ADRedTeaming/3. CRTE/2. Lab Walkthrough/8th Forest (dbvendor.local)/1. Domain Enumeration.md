...
<snip>
```
- We have found an interesting ACL showing that `db\srvdba` has `GenericAll` permissions on `dbvendor\dbXsvc` accounts. We can abuse that on [[3. CRTE/2. Lab Walkthrough/8th Forest (dbvendor.local)/2. Kill Chain|2. Kill Chain]] to gain control of that account.